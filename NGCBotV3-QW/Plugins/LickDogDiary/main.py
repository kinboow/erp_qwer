...
async def onLoad():
    logger.info(f"[{pluginConfig.get('name', 'LickDogDiary')}] 插件已加载")
    logger.info(f"触发词: {pluginConfig.get('settings', {}).get('keywords', [])}")

async def onUnload():
    logger.info(f"[{pluginConfig.get('name', 'LickDogDiary')}] 插件已卸载")
...
async def handle_text_message(data: dict):
    ...
    # 获取舔狗日记
    diary_text = await get_lickdog_diary(settings.get('apiUrl', ''))

    if diary_text:
        if is_group:
            await bot.sendAtText(robotId, reply_target, ' ' + diary_text, [sender])
        else:
            await bot.sendText(robotId, reply_target, diary_text)
    else:
        await bot.sendText(robotId, reply_target, "获取舔狗日记失败，请稍后再试~")

async def get_lickdog_diary(api_url: str) -> str:
    try:

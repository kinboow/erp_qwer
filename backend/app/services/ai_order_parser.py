...
# ==========================================================================
# @消息预筛选智能体 — At-Message Pre-check（严格过滤纯意图词）
# ==========================================================================
ORDER_AT_PRECHECK_SYSTEM_PROMPT = """你是一个服装行业报货信息预筛选助手。

客户在群里 @了机器人发送了一条文字消息。你需要判断这条 @消息是否包含**实际的报货数据**。

你会收到三段内容（不一定每段都有）：
- **上文**：触发消息之前的聊天记录（仅供参考上下文）
- **★ 触发消息**：客户@机器人的主消息（重点判断对象）
- **下文**：触发消息之后的聊天记录（仅供参考上下文）

注意：上下文中不会包含机器人自身发送的消息。

【核心判断标准】
必须在 ★ 触发消息本身中，至少出现以下之一：
- **具体的货号/款号**（如 82761、A1234、弯刀裤 等产品编号或名称）
- **具体的尺码**（如 S、M、L、XL、均码、28、29 等）
...
- "帮我下一下"、"再来一单"
- "取消之前的"、"换一下"
- 以上这些只是在表达下单意图，但没有出现一个具体的货号或一个具体的尺码
→ 这些都不算报货信息

【以下情况 → is_order=true】
消息中至少出现了一个具体的货号或一个具体的尺码：
- "@机器人 82761 黑色 M 10件" → 是（有货号+尺码）
- "@机器人 帮我下一下 82761" → 是（有货号）
- "@机器人 黑色 M码 10件" → 是（有尺码）
- "@机器人 弯刀裤 要10件" → 是（有产品名称）

注意：只要有一个货号或一个尺码就算 is_order=true，不需要信息完整。
信息是否完整由后续的验证智能体判断，你只需要判断是否包含实际报货数据。
...
客户在群里发送了一条报货信息（文字、图片或表格），你需要结合报货前后的聊天上下文，判断这笔订单的意图。

你会收到三段内容（不一定每段都有）：
- **上文**：触发消息之前的聊天记录（仅供参考上下文）
- **★ 触发消息**：客户发送的报货主消息（判断对象）
- **下文**：触发消息之后的聊天记录（仅供参考上下文）
...
class AIOrderParser:
    ...
    # ------------------------------------------------------------------
    # @消息预筛选：严格过滤纯意图词，只有含实际报货数据才放行
    # ------------------------------------------------------------------
    async def precheck_at_order(
        self,
        context_messages: list[dict[str, Any]],
        db: Optional[Session] = None,
    ) -> dict[str, Any]:
        """@消息预筛选：判断@消息是否包含实际报货数据（至少一个货号或尺码）。

        参数:
            context_messages: 包含前后上下文 + @消息本身的消息列表

        返回:
            {"is_order": True/False, "reason": "..."}
        """
        cfg = self._load_config(db)
        self._ensure_enabled(cfg)
        model = cfg["model"]  # 纯文本，不需要视觉模型

        user_parts = self._build_multimodal_parts(
            context_messages,
            prefix="以下是客户在群聊中@机器人的消息及前后上下文，请判断@消息是否包含实际报货数据：",
        )
        ...
        try:
            result = await self._chat(
                model,
                [
                    {"role": "system", "content": ORDER_AT_PRECHECK_SYSTEM_PROMPT},
                    {"role": "user", "content": user_parts},
                ],
                db=db,
                caller="precheck_at_order",
            )
            return {
                "is_order": bool(result.get("is_order", False)),
                "reason": str(result.get("reason", "")),
            ...
            logger.warning("@消息预筛选失败: %s", exc)
            return {"is_order": True, "reason": f"预筛选异常，保守放行: {exc}"}

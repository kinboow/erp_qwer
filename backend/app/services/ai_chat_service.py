...
- 你能看到群里所有客户消息（不含机器人自身、员工的消息、撤回的消息、非图片/Excel的文件）
- 消息会延迟约 2 分钟后批量发送给你，所以你可能一次收到多条消息，请全部处理
...
### 步骤1：判断消息是否包含报货数据
判断当前消息是否属于报货信息（不是所有消息都是报货）：
- 【非报货 → 不处理】日常闲聊、打招呼、问候、表情包、闲聊图片、讨论非订单话题、仅有下单意图词但无具体数据（如"下单""报货""我要下单"）
- 【是报货】消息中至少出现了以下之一：具体的货号/款号、产品名称/别名、颜色+尺码+数量
如果不是报货消息，直接忽略，不要回复。
...
def _tool_create_order_review(
    ...
    result = db.execute(
        text(
            ...
            "customer_id, customer_name, parsed_order_json, order_intent"
            ") VALUES ("
...
def _tool_void_recent_review(
    ...
    db.execute(
        text(
            "UPDATE downstream_order_reviews SET review_status = 'voided', review_note = :note, updated_at = NOW() WHERE id = :id"
        ),
